...
mutable struct DLTV
    ...
    """
    function DLTV(nx::Int, nu::Int, np::Int,
                  nv::Int, N::Int)::DLTV
        A = Array{Float64, 3}(undef, nx, nx, N-1)
        Bm = Array{Float64, 3}(undef, nx, nu, N-1)
        Bp = Array{Float64, 3}(undef, nx, nu, N-1)
        F = Array{Float64, 3}(undef, nx, np, N-1)
        r = Matrix{Float64}(undef, nx, N-1)
        E = Array{Float64, 3}(undef, nx, nv, N-1)
        timing = 0.0
