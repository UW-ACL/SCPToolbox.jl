...
if isdefined(@__MODULE__, :LanguageServer)
    ...
    using .ScpTrajOptUtils.ScpTypes
end
...
const RealValue = ScpTypes.RealValue
...
function plot_prisms!(ax::PyPlot.PyObject,
    ...
    for i = 1:length(H)
        Hi = H[i]
        x, y = axes
        vertices = Matrix{Float64}([Hi.l[x] Hi.u[x] Hi.u[x] Hi.l[x] Hi.l[x];
...
function plot_convergence(history, name::String)::Nothing
    ...
    Nnu = length(ud[1])
    np = length(p[1])
    X = Matrix{Float64}(undef, Nnx+Nnu+np, num_iter)
    for i = 1:num_iter
        X[:, i] = vcat(xd[i], ud[i], p[i])
    end
    DX = Vector{Float64}([norm(X[:, i]-X[:, end])/norm(X[:, end])
                          for i=1:(num_iter-1)])
    ...
    iters = ScpTypes.IntVector(1:num_iter)
    ...
    if num_iter<=15
        ...
        xticks = ScpTypes.IntVector(1:step:num_iter)
    end
