...
"""
function plot_trajectory_history(mdl::StarshipProblem, history::SCPHistory)::Nothing
...
function plot_final_trajectory(mdl::StarshipProblem, sol::SCPSolution)::Nothing
    ...
    v_nrm = matplotlib.colors.Normalize(vmin = minimum(speed), vmax = maximum(speed))
    ...
    plt.colorbar(v_cmap, aspect = 40, label = "Velocity [m/s]")
...
function plot_velocity(mdl::StarshipProblem, sol::SCPSolution)::Nothing
    ...
    ct_vel = hcat([sample(sol.xc, τ)[mdl.vehicle.id_v] for τ in ct_τ]...)

    for i = 1:2
        ax.plot(ct_time, ct_vel[i, :], color = xy_clrs[i], linewidth = 2)
    end
...
Args:
    mdl: the starship problem parameters.
    sol: the trajectory solution. =#
function plot_thrust(mdl::StarshipProblem, sol::SCPSolution)::Nothing
    ...
    ct_τ = RealVector(LinRange(0.0, 1.0, ct_res))
    ct_time = map(τ2t, ct_τ)
    ct_thrust = RealVector([sample(sol.uc, τ)[mdl.vehicle.id_T] * scale for τ in ct_τ])
    ax.plot(ct_time, ct_thrust, color = clr, linewidth = 2)
...
Args:
    mdl: the starship problem parameters.
    sol: the trajectory solution. =#
function plot_gimbal(mdl::StarshipProblem, sol::SCPSolution)::Nothing
    ...
    ct_gimbal_delayed =
        RealVector([sample(sol.xc, τ)[mdl.vehicle.id_δd] * scale for τ in ct_τ])
    ax.plot(
        ct_time,
        ct_gimbal_delayed,
        color = "#db6245",
    ...
    ct_gimbal = RealVector([sample(sol.uc, τ)[mdl.vehicle.id_δ] * scale for τ in ct_τ])
    ax.plot(ct_time, ct_gimbal, color = clr, linewidth = 2)
    ...
    ax.plot(ct_time, ct_δdot, color = clr, linewidth = 2, zorder = 100)
...
"""
function _starship__plot_glideslope(
    ax::PyPlot.PyObject,
    mdl::StarshipProblem;
    alt::RealValue = 200.0,
)::Nothing
...
"""
function _starship__plot_switch_time(ax::PyPlot.PyObject, t1::RealValue)::Nothing
    ax.axvline(
        x = t1,
        color = "black",
        linestyle = "--",
