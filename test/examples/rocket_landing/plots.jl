...
function plot_thrust(rocket::Rocket, sol::Solution, sim::Solution)::Nothing
    ...
    max_thrust_z =
        squeeze(mapslices(T -> sqrt((max_thrust)^2 - T[1]^2 - T[2]^2), T_dt, dims = 1))
    min_thrust_z = squeeze(
        mapslices(T -> sqrt(max(0, (min_thrust)^2 - T[1]^2 - T[2]^2)), T_dt, dims = 1),
    )
    ...
    ax = setup_axis!(111, xlabel = "Time [s]", ylabel = "Thrust [kN]", tight = "both")
    ...
    leg = ax.legend(framealpha = 0.8, fontsize = 8, loc = "lower left")
    leg.set_zorder(200)
...
function plot_mass(rocket::Rocket, sol::Solution, sim::Solution)::Nothing
    ...
    ax = setup_axis!(111, xlabel = "Time [s]", ylabel = "Mass [t]", tight = "both")
...
- `sim`: the continuous-time post-processed simulation.
"""
function plot_velocity(rocket::Rocket, sol::Solution, sim::Solution)::Nothing
...
- `sim`: the continuous-time post-processed simulation.
"""
function plot_pointing_angle(rocket::Rocket, sol::Solution, sim::Solution)::Nothing
    ...
    # Continuous-time thrust
    lbl =
        "\$\\arccos(\\hat e_z^{\\scriptscriptstyle" * "\\mathsf{T}}T_c(t)/\\|T_c(t)\\|_2)\$"
    ax.plot(
        t_ct,
        θ_ct,
        linewidth = 1.5,
    ...
    # Discrete-time thrust
    lbl =
        "\$\\arccos(\\hat e_z^{\\scriptscriptstyle" *
...
- `sim`: the continuous-time post-processed simulation.
"""
function plot_position(rocket::Rocket, sol::Solution, sim::Solution)::Nothing
    ...
    r_yz_ct = r_ct[[2; 3], :]
    v_cmap =
        generate_colormap("inferno"; minval = minimum(v_nrm_ct), maxval = maximum(v_nrm_ct))
    T_lw = 1.25
    T_lw_alpha = 0.8
    ...
    gspec = fig.add_gridspec(ncols = 2, nrows = 1, width_ratios = [0.015, 1])
    ...
    for k = 1:N-1
        ...
        thrust_whisker_tip = thrust_whisker_base + scale_factor * T_xz_dt[:, k]
    ...
    ax.fill(x_verts, y_verts, facecolor = Red, alpha = 0.5, edgecolor = "none")
    ax.plot(
        x_verts,
        y_verts,
        linestyle = "--",
    ...
    leg = ax.legend(handles, labels, framealpha = 0.8, fontsize = 8, loc = "upper left")
    leg.set_zorder(200)
