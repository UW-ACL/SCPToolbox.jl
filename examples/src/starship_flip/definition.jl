...
function starship_initial_guess(
    N::Int,
    pbm::TrajectoryProblem)::Tuple{RealMatrix,
                                   RealMatrix,
                                   RealVector}
    ...
    id_phase2 = IntVector(id_phase1[end]:N)
    ...
    x_guess[:, id_phase1] = hcat([
        sample(x1c, τ2t(τ)) for τ in τ_grid[id_phase1]]...)
    u_guess[:, id_phase1] = hcat([
    ...
    solve_trajectory = (t2) -> begin
        ...
        x0[3:4] = xs[veh.id_v]
        xf[3:4] = traj.vf
        @add_constraint(cvx, ZERO, "initial_condition",
                        (x[:, 1],), begin
                            local x0_var = arg[1]
                            x0_var-x0
                        end)
        @add_constraint(cvx, ZERO, "final_condition",
                        (x[:, end],), begin
                            local xf_var = arg[1]
                            xf_var-xf
                        end)
        ...
        for k = 1:N2-1
            ...
            @add_constraint(cvx, ZERO, "dynamics",
                            (xkp1, xk, uk, ukp1), begin
                                local xn, x, u, un = arg
                                xn-(A*x+Bm*u+Bp*un+r)
                            end)
        end
        ...
        for k = 1:N2
            ...
            @add_constraint(cvx, NONPOS, "pointy_end_up", (uk,), begin
                                local uk = arg[1]
                                veh.T_min1-dot(uk, env.ey)
                            end)
            @add_constraint(cvx, SOC, "tilt", (uk,), begin
                                local uk = arg[1]
...
function dynamics(t::RealValue,
                  ...
                  no_aero_torques::Bool=false)::RealVector
...
function set_convex_constraints!(pbm::TrajectoryProblem)::Nothing
    ...
    problem_set_X!(
        pbm, (t, k, x, p, pbm, ocp) -> begin
            ...
            @add_constraint(
                ocp, NONPOS, "no_climb", (v,), begin
                    local v = arg[1]
                    dot(v, env.ey)
                end)

            @add_constraint(
                ocp, NONPOS, "max_time", (tf1, tf2,), begin
                    local tf1, tf2 = arg
            ...
            @add_constraint(
                ocp, NONPOS, "min_time", (tf1, tf2,), begin
                    local tf1, tf2 = arg
                    local tf = tf1[1]+tf2[1]
                    traj.tf_min-tf
                end)
