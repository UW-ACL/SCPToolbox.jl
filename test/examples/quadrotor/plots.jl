...
"""
function plot_trajectory_history(mdl::QuadrotorProblem, history::SCPHistory)::Nothing
...
"""
function plot_final_trajectory(mdl::QuadrotorProblem, sol::SCPSolution)::Nothing
    ...
    plt.colorbar(v_cmap, aspect = 40, label = "Velocity \$\\|\\dot r\\|_2\$ [m/s]")
...
- `sol`: the trajectory solution.
"""
function plot_input_norm(mdl::QuadrotorProblem, sol::SCPSolution)::Nothing
    ...
    ct_acc_nrm = RealVector([norm(ct_acc_vec[:, k]) for k = 1:ct_res])
    ax.plot(ct_time, ct_acc_nrm, color = clr, linewidth = 2)

    # ..:: Norm of acceleration vector (discrete-time) ::..
    time = sol.td * sol.p[mdl.vehicle.id_t]
    acc_vec = sol.ud[mdl.vehicle.id_u, :]
    acc_nrm = RealVector([norm(acc_vec[:, k]) for k = 1:size(acc_vec, 2)])
...
# Arguments
    ...
    sol: the trajectory solution.
"""
function plot_tilt_angle(mdl::QuadrotorProblem, sol::SCPSolution)::Nothing
    ...
    ct_tilt = RealVector([acosd(_u[3, k] / norm(_u[:, k])) for k = 1:ct_res])
    ax.plot(ct_time, ct_tilt, color = clr, linewidth = 2)

    # ..:: Tilt angle (discrete-time) ::..
    time = sol.td * sol.p[mdl.vehicle.id_t]
    _u = sol.ud[mdl.vehicle.id_u, :]
    tilt = RealVector([acosd(_u[3, k] / norm(_u[:, k])) for k = 1:size(_u, 2)])
    ax.plot(
        time,
        tilt,
        linestyle = "none",
