...
if isdefined(@__MODULE__, :LanguageServer)
    include("ScpTrajOptUtils.jl")
    import .ScpTrajOptUtils.ScpTypes
end
...
"""
function skew(v::ScpTypes.RealVector)::ScpTypes.RealMatrix
    S = zeros(3, 3)
...
"""
function get_interval(x::ScpTypes.RealValue, grid::ScpTypes.RealVector)::Int
    k = sum(x.>grid)
    if k==0
	k = 1
    end
...
"""
function linterp(t::ScpTypes.RealValue,
                 f_cps::ScpTypes.RealArray,
                 t_grid::ScpTypes.RealVector)::Union{
                     ScpTypes.RealValue, ScpTypes.RealArray}
...
# Returns
    f_t: the function value at time t.
"""
function zohinterp(t::ScpTypes.Real,
                   f_cps::ScpTypes.RealArray,
                   t_grid::ScpTypes.RealVector)::Union{
                       ScpTypes.Real, ScpTypes.RealArray}
    if t>=t_grid[end]
        k = length(t_grid)
    else
        t = max(t_grid[1], min(t_grid[end], t)) # Saturate to time grid
...
function straightline_interpolate(
    v0::ScpTypes.RealVector,
    vf::ScpTypes.RealVector,
    N::ScpTypes.Int)::ScpTypes.RealMatrix
...
"""
function rk4(f::ScpTypes.Func,
             x0::ScpTypes.RealVector,
             tspan::ScpTypes.RealVector;
             full::Bool=false,
             actions::ScpTypes.SpecialIntegrationActions=
                 ScpTypes.SpecialIntegrationActions(undef, 0))::Union{
                     ScpTypes.RealVector, ScpTypes.RealMatrix}
...
function rk4(f::ScpTypes.Func,
             x0::ScpTypes.RealVector,
             tf::ScpTypes.RealValue,
             h::ScpTypes.RealValue;
             full::Bool=false,
             actions::ScpTypes.SpecialIntegrationActions=
                 ScpTypes.SpecialIntegrationActions(undef, 0))::Union{
                     ScpTypes.RealVector, ScpTypes.RealMatrix}
...
- `xp`: the next state.
"""
function rk4_core_step(f::ScpTypes.Func,
                       x::ScpTypes.RealVector,
                       t::ScpTypes.RealValue,
                       tp::ScpTypes.RealValue)::ScpTypes.RealVector
...
function rk4_generic(
    f::ScpTypes.Func,
    x0::ScpTypes.RealVector;
    tspan::Union{ScpTypes.RealVector, Nothing}=nothing,
    h::Union{ScpTypes.RealValue, Nothing}=nothing,
    full::Bool=false,
    actions::ScpTypes.SpecialIntegrationActions=
        ScpTypes.SpecialIntegrationActions(undef, 0))::Union{
            ScpTypes.RealVector, ScpTypes.RealMatrix}
...
"""
function trapz(f::AbstractVector, grid::ScpTypes.RealVector)
    N = length(grid)
    F = 0.0
    for k = 1:N-1
        δ = grid[k+1]-grid[k]
...
"""
function logsumexp(
    f::ScpTypes.RealVector,
    ∇f::Union{Nothing, Vector{Vector{Float64}}}=nothing;
    t::ScpTypes.RealValue=1.0)::Union{
        Tuple{ScpTypes.RealValue, ScpTypes.RealVector},
        ScpTypes.RealValue}
...
"""
function sigmoid(f::ScpTypes.RealVector,
                 ∇f::Union{Nothing, Vector{Vector{Float64}}}=nothing;
                 κ::ScpTypes.RealValue=1.0)::Union{
                     Tuple{ScpTypes.RealValue, ScpTypes.RealVector},
                     ScpTypes.RealValue}
...
"""
function indicator(f::ScpTypes.RealVector,
                   ∇f::Union{Nothing, Vector{Vector{Float64}}}=nothing;
                   κ1::ScpTypes.RealValue=1.0,
                   κ2::ScpTypes.RealValue=1.0)::Union{
                       Tuple{ScpTypes.RealValue, ScpTypes.RealVector},
                       ScpTypes.RealValue}
...
function or(predicates...;
            κ1::ScpTypes.RealValue=1.0,
            κ2::ScpTypes.RealValue=1.0,
            minval::ScpTypes.RealValue=-1.0,
            maxval::ScpTypes.RealValue=1.0)::Union{
                Tuple{ScpTypes.RealValue, ScpTypes.RealVector},
                ScpTypes.RealValue}
    ...
    if typeof(predicates[1])<:Tuple
        ...
        f = Vector{Float64}([scale(p[1]) for p in predicates])
        ∇f = [∇scale(p[2]) for p in predicates]
        OR, ∇OR = indicator(f, ∇f; κ1=κ1, κ2=κ2)
        return OR, ∇OR
    else
        f = Vector{Float64}([scale(p) for p in predicates])
